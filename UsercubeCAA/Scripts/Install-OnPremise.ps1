...
.EXAMPLE
    PS>./Install-OnPremise -ConfigurationPath "ConfOld"
    PS>./Install-OnPremise -cp "ConfOld"

    Installs a Usercube instance based on the conf from the folder 'ConfOld' instead of the folder 'Conf'.
#>
...
param (
    ...
    [Alias('cp')]
    [string]
    # Runs the installation process based on a given configuration path.
    # Valid paths are : Conf, Conf101.
    # 'Conf' runs the standard installation based on the folder Conf, while 'Conf101' runs the installation of the 101 version based on the folder Conf101.
    $ConfigurationPath = 'Conf'
)

. "$PSScriptRoot/Environment.ps1"

if ($ConfigurationPath -eq 'Conf')
{
  $JobId = "InitializationJob"
}

$NoCreateDatabase = $NoCreateDatabase -or $From -ne ''
$NoImportConfiguration = $NoImportConfiguration -or $From -eq 'LaunchServer' -or $From -eq $JobId
$NoLaunchServer = $NoLaunchServer -or $From -eq $JobId
...
{
  Launch-Automaton -jobIdentifier $JobId
}
