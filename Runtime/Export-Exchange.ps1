...
function New-Csv {
  ...
  $line = '"Command"' + $separator + '"' + $filterString.Replace($separator, '"' + $separator + '"') + '"'
  Write-Bytes -text $line -writer $stream -appendLine $true
}
...
function Add-Csv {
  ...
  for ($i = 0; $i -lt $entityCount; $i++) {
    ...
    Write-Bytes -text $msg[1] -writer $stream -appendLine $true
  }
...
function Write-Bytes() {
  param (
    ...
    [Parameter(Mandatory = $false)][bool]$appendLine
  )
  ...
  $bytes = [System.Text.Encoding]::UTF8.GetBytes($textToWrite)
